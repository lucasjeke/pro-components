...
const baseMenus = (siderMenus.value.slice(1) as unknown as MenuGroup[]).map(group => ({
  ...group,
        ...
        <a-row :gutter="[24, 24]">
          <template v-for="comp in group.children" :key="comp.key">
            <a-col :xs="24" :sm="12" :lg="8" :xl="6">
              <RouterLink :to="locale === 'zh-CN' ? comp.key : `/en-US${comp.key}`" style="text-decoration: none; color: inherit;">
