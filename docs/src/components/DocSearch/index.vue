...
import { AppstoreOutlined, FileTextOutlined, ProfileOutlined, SearchOutlined } from '@antdv-next/icons'
...
function clearResults(closePanel = true) {
  results.value = []
  if (closePanel)
    open.value = false
}
...
function handleSelect(item: SearchResultItem) {
  ...
  const target: RouteLocationRaw = hash ? { path, hash: `#${hash}` } : { path }
  router.push(target)
  searchValue.value = ''
  open.value = false
}
...
function isEventInsideSearch(event: Event) {
  const target = event.target
  if (!(target instanceof Node))
    return false
  return !!wrapperRef.value?.contains(target) || !!panelRef.value?.contains(target)
}

function isFocusInsideSearch() {
  const activeElement = document.activeElement
  if (!activeElement)
    return false
  return !!wrapperRef.value?.contains(activeElement) || !!panelRef.value?.contains(activeElement)
}
...
function handlePopoverOpenChange(nextOpen: boolean) {
  ...
  if (isFocus.value || isFocusInsideSearch())
    return
...
useEventListener(document, 'pointerdown', (event) => {
  if (!open.value || isEventInsideSearch(event))
    return
  open.value = false
})
...
<template>
  ...
  >
    <div ref="wrapperRef" v-bind="$attrs" @click="handleTriggerClick">
      <a-input
        v-model:value="searchValue"
        :classes="{
          root: `!rounded-5 !p-bs-1.5 !p-be-1.5 ${isFocus ? '' : '!bg-transparent !border-[rgb(217_,_217_,_217)]'}`,
        }"
      ...
      >
        <template #prefix>
          <SearchOutlined class="!color-[--ant-color-text-tertiary]" />
        </template>
        <template v-if="!isFocus" #suffix>
          <a-tag
            :classes="{
              root: '!rounded-5 !color-[rgba(42_,_46_,_54_,_0.45)] !bg-[rgba(42_,_46_,_54_,_0.06)] !p-is-2 !p-ie-2',
            }"
          >
            <a-space :size="3">
              <span>⌘</span>
    ...
    <template #content>
      <div ref="panelRef" class="ant-doc-search-panel">
        ...
        <div v-else-if="!results.length" class="ant-doc-search-panel-state">
          <a-empty :description="t('ui.docSearch.emptyText') " />
        </div>
