---
category: Components
title: ProDescriptions
subtitle: 定义列表
group: 数据展示
---

高级描述列表组件，提供一个更加方便快速的方案来构建描述列表。



## 何时使用 {#when-to-use}

ProDescriptions 的诞生是为了解决项目中需要写很多 Descriptions 的样板代码的问题，所以在其中封装了很多常用的逻辑。在 Vue 中既可以通过 `columns` 配置描述项，也可以使用 `ProDescriptionsItem` 子组件按 SFC 模板方式声明。

比如 ProDescriptions 封装了请求网络的行为，ProDescriptions 会将 props.params 中的数据默认带入到请求中，如果接口恰好与我们的定义相同，实现一个查询会非常简单。

```tsx
import axios from 'axios';
const fetchData = (params) =>
  axios.get<{
    data: T{};
  }>('https://proapi.azurewebsites.net/github/issues', {
    params,
  });
const keyWords = "Ant Design"
<ProDescriptions request={fetchData} />;
```

我们约定 request 拥有一个参数， `params` 会自带 props 中的 `params` 。类型如下:

```ts
(params: U) => RequestData;
```

对于请求回来的结果，ProDescriptions 也有一些约定，类型如下：

```ts
interface RequestData {
  data: Datum{};
  success: boolean;
}
```


## 代码演示 {#examples}

<demo-group>
  <demo src="./demo/basic.vue">基础定义列表</demo>
  <demo src="./demo/sfc.vue">SFC 模式</demo>
  <demo src="./demo/arrayDataIndex.vue">数组类型dataIndex</demo>
    <demo src="./demo/format.vue">根据配置格式化日期</demo>
  <demo src="./demo/request.vue">远程请求数据</demo>
  <demo src="./demo/columns.vue">columns配置</demo>
  <demo src="./demo/use-data-source.vue">dataSource 配置数据</demo>
  <demo src="./demo/editable.vue">可编辑的定义列表</demo>
</demo-group>


## API

### ProDescriptions

> 更多功能查看 antdv-next 的 [Descriptions](https://www.antdv-next.cn/components/descriptions)

| 属性 | 说明 | 类型 | 默认值 | 版本 |
| --- | --- | --- | --- | --- |
| title | 描述列表的标题，显示在最顶部 | `VueNode` | - | - |
| tooltip | 内容的补充描述，hover 后显示 | `string` | - | - |
| loading | 展示一个加载的骨架屏，骨架屏和 dom 不会一一对应 | `boolean` | - | - |
| extra | 描述列表的操作区域，显示在右上方 | `string` \| `VueNode` | - | - |
| bordered | 是否展示边框 | `boolean` | `false` | - |
| column | 一行的 `ProDescriptionsItems` 数量，可以写成像素值或支持响应式的对象写法 `{ xs: 1, sm: 2, md: 3}` | `number` | `3` | - |
| size | 设置列表的大小。可以设置为 `middle` 、`small`，或不填（只有设置 `bordered={true}` 生效） | `default` \| `middle` \| `small` | - | - |
| layout | 描述布局 | `horizontal` \| `vertical` | `horizontal` | - |
| onRequestError | 处理 request 的错误，默认会直接抛出错误 | `(error: Error) => void` | - |
| colon | 配置 `ProDescriptions.Item` 的 `colon` 的默认值 | `boolean` | `true` | - |
| request | 请求数据，与 `columns` 中的 `dataIndex` 配合使用 | `(params: U) => Promise<RequestData<T>>` | - | - |
| columns | 列定义，与 request 配合使用 [columns](https://www.antdv-next.cn/components/table#columns) | `ProDescriptionsItemProps[]` | - | - |
| editable | 编辑的相关配置 | [EditableConfig](#editable-编辑配置) | - | - |
| dataSource | 数据源 | `T[]` | - | - |

### editable 编辑配置 {#edit-config}

| 属性 | 描述 | 类型 | 默认值 |
| --- | --- | --- | --- |
| form | 可编辑表格的 form 实例，使用 `Form.useForm` 生成后使用 | `FormInstance` | - |
| formProps | 可以配置 form 的属性，但是不支持 onFinish | [`FormProps`](/components/form#proform) | - |
| editableKeys | 正在编辑的行，受控属性。 默认 `key` 会使用 `rowKey` 的配置，如果没有配置会使用 `index`，建议使用 rowKey | `Key[]` | - |
| onChange | 行数据被修改的时候触发 | `(editableKeys: Key[], editableRows: T[]) => void` | - |
| onSave | 保存一行的时候触发 | `(key: Key, row: T,originRow:T,newLine?:newLineConfig) => Promise<any>` | - |
| saveText | 保存一行的文字 | `VueNode` | `保存` |
| onDelete | 删除一行的时候触发 | `(key: Key, row: T) => Promise<any>` | - |
| deleteText | 删除一行的文字 | `VueNode` | `删除` |
| onCancel | 取消编辑一行时触发 | `(key: Key, row: T,originRow:T,newLine?:newLineConfig) => Promise<any>` | - |
| cancelText | 取消编辑一行的文字 | `VueNode` | `取消` |
| actionRender | 自定义编辑模式的操作栏 | `(row: T, config: ActionRenderConfig,defaultDom) => VueNode[]` | - |
| deletePopconfirmMessage | 删除时弹出的确认框提示消息 | `VueNode` | `删除此项？` |
| onlyOneLineEditorAlertMessage | 只能编辑一行的的提示 | `VueNode` | `只能同时编辑一行` |
| onlyAddOneLineAlertMessage | 只能同时新增一行的提示 | `VueNode` | `只能新增一行` |

### ProDescriptions.Item

| 参数 | 说明 | 类型 | 默认值 |
| --- | --- | --- | --- |
| label | 内容的描述 | ReactNode | - |
| tooltip | 内容的补充描述，hover 后显示 | string | - |
| ellipsis | 是否自动缩略 | `boolean` | - |
| copyable | 是否支持复制 | `boolean` | - |
| span | 包含列的数量 | number | 1 |
| valueType | 格式化的类型 | `ValueType` | - |
| valueEnum | 当前列值的枚举 [valueEnum](/components/table#valueenum) | `Record` | - |
| request | 从网络请求枚举数据 | `()=>Promise<{[key:string`\|`number]:any}>` | - |
| dataIndex | 返回数据的 key 与 ProDescriptions 的 request 配合使用，用于配置式的定义列表 | `Text` \| `Text[]` | - |
| editable | 在编辑表格中是否可编辑的，函数的参数和 table 的 render 一样 | `false` \| `(text: any, record: T,index: number) => boolean` | true |

> span 是 Description.Item 的数量。 span={2} 会占用两个 DescriptionItem 的宽度。

### ProDescriptionsInstance

在进行了操作，或者 tab 切换等时候我们需要手动触发一下描述列表的更新，纯粹的 props 很难解决这个问题，所以我们提供一个 ProDescriptionsInstance 来支持一些默认的操作。

```tsx
const descriptionsRef = useRef<ProDescriptionsInstance<Record<string, any>>>();

// 两秒刷新一次
  setInterval(() => {
    descriptionsRef.value.reload();
  }, 2000);

// hooks 绑定
<ProDescriptions ref={descriptionsRef} />;>;
```

`ProDescriptionsInstance` 还支持了一些别的行为，某些时候会减少的你的编码成本。

```tsx | pure
// 刷新
descriptionsRef.value.reload();
```
