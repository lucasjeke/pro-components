...
import { ProForm, ProFormDependency, ProTable } from '@antdv-next1/pro-components'
...
<template>
  <div class="p-6">
    <ProForm
      :model="{
        columnsInfo: defaultData,
        table: defaultData,
      }"
    >
      <ProFormDependency :name="['columnsInfo']">
        <template #default="{ values: { columnsInfo } }">
          <a-tabs
            :style="{
              padding: '0 8px',
              maxWidth: '100%',
            }"
            :items=" (columnsInfo || []).map((item: { name?: string, columnsInfo?:({
              name: string
            })[]}, index: number) => {
              return {
                key: item.name || '',
                label: h('div', { style: { padding: '4px' } }, item.name),
                content: h('div', {
                  style: {
                    display: 'flex',
                    maxWidth: '1024px',
