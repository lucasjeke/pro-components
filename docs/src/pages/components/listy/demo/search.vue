...
<template>
  <div class="p-6">
    <ProListy
      :tool-bar-render="() => [h(Button, { key: 3, type: 'primary' }, () => '新建')]"
      :search="{}"
      row-key="name"
      header-title="基础列表"
      ...
      :columns="[
        ...
        { dataIndex: 'title', listSlot: 'description', search: false },
        {
          ...
          listSlot: 'subTitle',
          search: false,
          render: (_, row) => h(Space, { size: 0 }, () => row.labels?.map((label: { name: string }) => h(Tag, {
            color: 'blue',
            key: label.name,
          }, () => label.name))),
        },
        {
          ...
          render: (_, row) => h(Space, {
            ...
            h('a', { href: row.url, target: '_blank', rel: 'noopener noreferrer', key: 'link' }, '详情'),
            h('a', { href: row.url, target: '_blank', rel: 'noopener noreferrer', key: 'view' }, '查看'),
          ]),
        },
        {
          title: '状态',
