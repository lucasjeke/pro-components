...
const valueTypeOptions = reactive([
...
function getValueTypeInitialValue(valueType: ProFieldValueType) {
  const option = valueTypeOptions.find(item => item.value === valueType)
  return cloneValueTypeInitialValue(option?.initialValue)
}

function createValueTypeModel(valueType: ProFieldValueType) {
  const fieldName = valueType || 'text'
  const initialValue = getValueTypeInitialValue(valueType)
  return {
    [fieldName]: initialValue,
    [`${fieldName}Readonly`]: cloneValueTypeInitialValue(initialValue),
  }
}

const valueType = shallowRef<ProFieldValueType>('textarea')
const formModel = reactive<Record<string, any>>(createValueTypeModel(valueType.value))
const currentInitialValue = computed(() => getValueTypeInitialValue(valueType.value))
const editorDataIndex = computed(() => valueType.value || 'text')
const readonlyDataIndex = computed(() => `${editorDataIndex.value}Readonly`)
const schemaFormRef = useTemplateRef<ProFormRef<Record<string, any>>>('schemaFormRef')
function resetValueTypeModel(nextValueType = valueType.value) {
  Object.keys(formModel).forEach((key) => {
    Reflect.deleteProperty(formModel, key)
  })
  Object.assign(formModel, createValueTypeModel(nextValueType))
}

function handleValueTypeChange(nextValueType: ProFieldValueType) {
  if (!isValueTypeOptionValue(nextValueType))
    return
  valueType.value = nextValueType!
  resetValueTypeModel(nextValueType)
}
const columns = computed(() => [
  {
    ...
    columns: [
      {
        key: `${valueType.value}-editor`,
        valueType: valueType.value,
        title: '编辑器',
        name: editorDataIndex.value,
        dataIndex: editorDataIndex.value,
...
function hasOwnValue(values: Record<string, any>, key: string) {
...
function handleFormValuesChange(
  ...
  const editorName = editorDataIndex.value
  const readonlyName = readonlyDataIndex.value

  // 关键：第二次触发时 changedValues 是 { xxxReadonly: value }
  // 不是左侧编辑字段，所以直接忽略
  if (!hasOwnValue(changedValues, editorName))
    return
