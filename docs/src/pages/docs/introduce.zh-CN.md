...
## ProComponents Vue 的理念

Antdv Next 定义了基础的设计规范，对应也提供了大量的基础组件。但是对于中后台类应用，我们希望提供更高程度的抽象，提供更上层的设计规范，并且提供相应的组件使得开发者可以快速搭建出高质量的页面。

在 ProComponents Vue 中我们内置了一系列的设计规范，预设了常用的逻辑。在这个基础上我们同样提供了灵活的支持，比如对于 ProTable 来说你也可以把它完全当做 Antdv Next 的 Table 来用，对于 ProForm 来说你也可以直接使用 Antdv Next 的基础组件或者你的自定义组件。我们希望通过 Pro 系列组件提供快速高效搭建高质量中后台应用的能力，进一步扩展 Antdv Next 的能力，欢迎使用并提出宝贵的意见。
...
> 为了渐进式使用我们也提供了与 Antdv Next 相同的 api，完全可以降级成为一个 Antdv Next 的 table 使用。
...
- 在你的公司或个人项目中使用 Antdv Next Pro 和 ProComponents Vue。
- 通过 [Issue](http://github.com/lucasjeke/pro-components/issues) 报告 bug 或进行咨询。
- 提交 [Pull Request](http://github.com/lucasjeke/pro-components/pulls) 改进 ProComponents Vue 的代码。
...
- tests              * 编写测试用例的地方
- scripts            * 开发或者部署所用的脚本
- .prettierrc.cjs    * prettier 的相关配置
- .eslint.config.ts  * eslint 的配置
- package.json       * 项目的配置
- tsconfig.base.json * typescript 的基础配置
- tsconfig.json      * typescript 的配置
- tsconfig.test.json * typescript 的测试配置
...
在 packages 文件夹中包含了我们所有的组件，每个组件一般都有一个 `src`，`package.json` 和 `README.md`。
...
ProComponents Vue 基于 Antdv Next 来开发，为了与 Antdv Next 的生态保持兼容性，我们要求覆盖 Antdv Next 的样式必须要使用 `${token.antCls}` 变量来生成类名，在 js 中使用如下代码来配置实现。

```ts
const  config = useConfig();
const prefixCls = computed(() => config.getPrefixCls('pro-${包名}'));
```
